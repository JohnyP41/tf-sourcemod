...
public cvhook_allowpyro(Handle:cvar, const String:oldVal[], const String:newVal[]) {
	...
	if (cvar_allowpyro) {
		PrintToChatAll("\x04[%s]\x01 Klasa: \x03Pyro\x01 nie jest dostępna w zespole RED", PLUGIN_NAME);
	} else {
		PrintToChatAll("\x04[%s]\x01 Klasa: \x03Pyro\x01 nie jest dostępna w zespole RED", PLUGIN_NAME);
	}
}
...
public cvhook_allowengineer(Handle:cvar, const String:oldVal[], const String:newVal[]) {
	...
	if (cvar_allowengineer) {
		PrintToChatAll("\x04[%s]\x01 Klasa: \x03Engineer\x01 nie jest dostępna w zespole RED", PLUGIN_NAME);
	} else {
		PrintToChatAll("\x04[%s]\x01 Klasa: \x03Engineer\x01 nie jest dostępna w zespole RED", PLUGIN_NAME);
	}
}
		...
		if (client == hidden) {
			...
			if (hiddenAway) {
				...
				if (hiddenAwayTime > HIDDEN_AWAY_TIME) {
					...
					PrintToChatAll("\x04[%s]\x01 \x03Ukryty\x01 został zabity bo się nie ruszał", PLUGIN_NAME);
					continue;
				}
			}
			...
			if (hiddenInvisibility > 0.0) {
				...
				if (hiddenInvisibility < 0.0) {
					...
					ForcePlayerSuicide(client);
					PrintToChatAll("\x04[%s]\x01 \x03Ukryty\x01 stracił swoje moce!", PLUGIN_NAME);
					continue;
				}
			}
...
public Action:player_spawn(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if ((client == hidden) && (class != TFClass_Spy)) {
		...
		if ((client == g_lastHidden) && (g_lastHiddenSavedClass != TFClass_Unknown)) {  //if we haven't set them to their pre-hidden class choice
			...
			PrintToChat(client, "\x04[%s]\x01 Nie możesz używać tej klasy w zespole RED", PLUGIN_NAME);
			CreateTimer(0.1, Timer_Respawn, client);
		}
	}
}
...
public Action:player_death(Handle:event, const String:name[], bool:dontBroadcast) {
	...
	if (victim == hidden) {
		...
		if (IsValidClient(attacker) && attacker != hidden) {
			...
			PrintToChatAll("\x04[%s]\x01 \x03Ukryty\x01 został zabity przez \x03%N\x01!", PLUGIN_NAME, attacker);
		}
	} else {
		if (IsValidClient(hidden) && attacker == hidden) {
			...
			PrintToChatAll("\x04[%s]\x01 \x03Ukryty\x01 zabił \x03%N\x01 i zjadł jego ciało", PLUGIN_NAME, victim);
...
public Action:Cmd_build(client, String:cmd[], args)
{
	...
	if (building == _:TFObject_Sentry) {
		PrintToChat(client, "\x04[%s]\x01 Nie możesz budować działek w tym trybie.", PLUGIN_NAME);
...
public Action:Cmd_NextHidden(client, args) {
	...
	PrintToChat(client, "\x04[%s]\x01 Ukrytym będzie \x03%N\x01", PLUGIN_NAME, target);
...
public Action:Command_EnableHidden(client, args) {
	...
	ReplyToCommand(client, "[%s] Włączono.", PLUGIN_NAME);
	return Plugin_Handled;
}
...
public Action:Command_DisableHidden(client, args) {
	...
	ReplyToCommand(client, "[%s] Wyłączono.", PLUGIN_NAME);
	return Plugin_Handled;
}
...
stock SelectHidden() {
	...
	PrintToChat(hidden, "\x04[%s]\x01 Jesteś \x03Ukrytym\x01! zabij zespół RED!", PLUGIN_NAME);
	PrintToChat(hidden, "\x04[%s]\x01 Naduś \x04attack2\x01 aby skoczyć; \x04reload\x01 aby ogłuszyć wrogów.", PLUGIN_NAME);
...
stock ShowHiddenHP(Float:duration) {
	...
	if (Client_IsIngame(hidden)) {
		ShowHudText(hidden, 0, "Zdrowie Ukrytego: %.1f%%", perc);
		
		SetHudTextParams(-1.0, 0.325, duration, 255, 255, 255, 255);
		ShowHudText(hidden, 1, "Wytrzymałość: %.0f%%", hiddenStamina / HIDDEN_STAMINA_TIME * 100.0);
		
		#if defined HIDDEN_BOO
		SetHudTextParams(-1.0, 0.35, duration, 255, 255, 255, 255);
		ShowHudText(hidden, 2, "Ogłuszenie: %.0f%%", 100.0 - hiddenBoo / HIDDEN_BOO_TIME * 100.0);
		#endif
	}
}
